    ...
    public static function connect_hawk($platform, $params = [],$extend, $keeptime = 0)
    {
        ...
        if ($third)
        {
            ...
            if (!$user)
            {
                ...
                // 默认注册一个会员
                $result = $auth->register($username, $password, $username . '@her-family.com', '', $extend, $keeptime);
                if (!$result)
                {
                    return FALSE;
                }
                $user = $auth->getUser();
                $fields = ['username' => 'u' . $user->id, 'email' => 'u' . $user->id . '@her-family.com'];
                ...
                $fields['group_id'] = 3;    //默认每个人 进来 就是求职者
                // 更新会员资料
                $user->save($fields);
