...
class qp_dirty
{
     ...
     */
    function __construct( $filename = EXTEND_PATH.'/dirty/dirty.txt', $lev = 0, $sep = '|', $note = '#', $mark = '$' )
    {
    ...
    function replace_dirty($source,&$target)
    {
        ...
        {
            //echo $key;echo "<br>";
            $num =mb_strpos($source,$key);
            if($num === 0 || $num>0 ){

                $target = str_replace($key,$replace_str,$target);
                //$key = iconv("UTF-8", "GB2312", "$key");

                $this->dirty_words_list[] = $key;
            }
            
        }
        return $this->dirty_words_list;
    }
}
